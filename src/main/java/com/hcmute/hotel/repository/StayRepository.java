...
public interface StayRepository extends JpaRepository<StayEntity,String> {
    ...
    @Query(value = "SELECT stays.id, min(price), max(guest_number) FROM stays inner join rooms on stays.id = rooms.stay_id WHERE (province = ?1 OR ?1 IS NULL) AND (price between ?2 and ?3 ) AND (((?4 BETWEEN time_open AND time_close) OR ?4 IS NULL) AND ((?5 BETWEEN time_open AND time_close) OR ?5 IS NULL)) AND ( guest_number>=?6) AND (status = ?7 OR ?7 = 'NULL') AND (hidden = ?8 OR ?8 IS NULL) AND (CONCAT(name, ' ', address_description) LIKE CONCAT('%', COALESCE(?9, ''), '%')) AND (COALESCE((?10), NULL) IS NULL OR stays.id IN (SELECT stay_id FROM stay_amenities WHERE amenities_id IN (?11) GROUP BY stay_id HAVING COUNT(DISTINCT amenities_id) = ?12)) group by stays.id order by concat('room.', ?13, ' ', ?14)", nativeQuery = true)
